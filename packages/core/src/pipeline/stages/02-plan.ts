...
export async function runPlanStage(ctx: PipelineContext): Promise<void> {
  ...
  // 1. Dynamic fallback logic: if mode is online but the project is local and has localhost or relative calls, fallback to offline
  let mode = ctx.config.mode ?? "online";
  const detection = ctx.detection!;

  if (mode === "online" && !detection.isLiveUrl) {
    const hasLocalCalls = detection.warnings.some(
      (w) => w.includes("connectivity") || w.includes("localhost") || w.includes("relative")
    );
    if (hasLocalCalls) {
      ctx.log(
        "warn",
        "Pipeline detected localhost/local API calls or relative /api/ endpoints. " +
        "These will fail in standard online mode without a hosted backend URL. " +
        "Automatically falling back to 'offline' mode to generate a local SQLite + Express backend.",
        STAGE
      );
      mode = "offline";
    }
...
async function buildMigrationPlan(
  ...
  if (detection.isLiveUrl || mode === "online") {
