...
function generateInline(plan: FileGeneratePlan): string {
  ...
  switch (plan.generatorType) {
    ...
    case "local-auth-client":
      return generateLocalAuthClient();
    case "sync-engine":
...
function generateLocalAuthClient(): string {
  return `/**
...
async function generateCleanEnv(ctx: PipelineContext): Promise<void> {
  ...
  const isHybrid = ctx.config.mode === "hybrid";
  ...
  for (const f of sourceEnvFiles) {
    try {
      const content = await fs.readFile(path.join(ctx.sourceDir, f), "utf-8");
      sourceChunks.push(`# From ${f}`);
      sourceChunks.push(content.trimEnd());
    } catch { /* try next */ }
  }

  const sourceContent = sourceChunks.join("\n\n");
  ...
  if (isOnline || isHybrid) {
    ...
    const outputLines: string[] = [
      `# Generated by WebToApp - desktop app environment (${ctx.config.mode} mode)`,
      "# Cloud credentials are preserved so selected-mode behavior can match the web app.",
      "",
    ];
    ...
    if (isHybrid) {
      if (!sourceContent.includes("VITE_LOCAL_API")) {
        outputLines.push("VITE_LOCAL_API=true");
      }
      if (!sourceContent.includes("VITE_API_PORT")) {
        outputLines.push(`VITE_API_PORT=${ctx.config.backend?.port ?? 3001}`);
      }
      if (!sourceContent.includes("VITE_HYBRID_SYNC")) {
        outputLines.push("VITE_HYBRID_SYNC=true");
      }
    }

    const destPath = path.join(ctx.outputDir, ".env");
    await fs.writeFile(destPath, outputLines.join("\n") + "\n", "utf-8");
    ctx.log("info", `Generated .env (${ctx.config.mode} mode - cloud credentials preserved)`, STAGE);
  } else {
