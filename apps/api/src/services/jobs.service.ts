...
const PLAN_LIMITS: Record<Plan, { monthlyLimit: number | null; platforms: string[]; priority: number }> = {
...
export async function createJob(input: {
  ...
  if (invalidPlatforms.length > 0) {
    throw new PlanLimitError(
      `Your ${input.plan} plan cannot build for: ${invalidPlatforms.join(", ")}. Upgrade at /billing.`
    );
  }
...
export async function getJob(userId: string, jobId: string): Promise<Job & { artifacts: Artifact[] }> {
