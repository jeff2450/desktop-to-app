...
export default function DashboardPage() {
  ...
  const activeJobs = conversions.filter(c => 
    !["done", "failed", "cancelled"].includes(c.status)
  );
      ...
      <div className="p-8 space-y-8 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <Card className="lg:col-span-2 bg-zinc-900/50 border-zinc-800 backdrop-blur-sm overflow-hidden relative">
            ...
            <CardHeader>
              <div className="flex justify-between items-start">
                <div>
                  <CardTitle className="text-xl font-bold">Monthly Usage</CardTitle>
            ...
            <CardContent>
              {loading ? (
                ...
                <div className="space-y-6">
                  <div className="flex justify-between items-end">
                    <div className="space-y-1">
                      <p className="text-3xl font-bold text-white">
                        {usage.usage} <span className="text-zinc-500 text-lg font-normal">/ {usage.limit === 9999 ? "∞" : usage.limit}</span>
                      </p>
                      <p className="text-xs text-zinc-500">Conversions performed this month</p>
                    </div>
                  ...
                  <Progress value={usage.percentUsed} className="h-2 bg-zinc-800 [&>div]:bg-indigo-500" />
                </div>
              ) : (
          ...
          <Card className="bg-indigo-600 border-indigo-500 shadow-[0_0_30px_rgba(99,102,241,0.2)]">
            ...
            <CardContent>
              <Button asChild className="w-full bg-white text-indigo-600 hover:bg-indigo-50 shadow-lg font-bold py-6 rounded-xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          <div className="md:col-span-3 space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-bold uppercase tracking-widest text-zinc-500">Active Jobs</h3>
            ...
            {loading ? (
              [1, 2].map(i => <Skeleton key={i} className="h-24 w-full bg-zinc-900 border-zinc-800 rounded-2xl" />)
            ) : activeJobs.length > 0 ? (
              <div className="space-y-4">
                {activeJobs.map(job => (
                    ...
                    <div className="group bg-zinc-900/50 border border-zinc-800 p-5 rounded-2xl hover:border-indigo-500/50 transition-all flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-zinc-500 uppercase font-medium tracking-tighter">Status: {job.status}</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-4">
                        <div className="w-32 h-1.5 bg-zinc-800 rounded-full overflow-hidden">
                          <div className="h-full bg-indigo-500 animate-progress-flow" style={{ width: '40%' }} />
                        </div>
              ...
              <div className="p-12 border border-dashed border-zinc-800 rounded-3xl text-center">
                <p className="text-zinc-500 text-sm">No active conversions at the moment.</p>
              </div>
            )}
          </div>
        ...
        <section className="space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-bold uppercase tracking-widest text-zinc-500">Recent Conversions</h3>
              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="border-b border-zinc-800/50">
                    <th className="px-6 py-4 text-xs font-bold text-zinc-500 uppercase">App Name</th>
                    <th className="px-6 py-4 text-xs font-bold text-zinc-500 uppercase">Targets</th>
                ...
                <tbody className="divide-y divide-zinc-800/30">
                  {loading ? (
                    [1, 2, 3].map(i => (
                      <tr key={i}>
                        <td className="px-6 py-4"><Skeleton className="h-4 w-32 bg-zinc-800" /></td>
                        <td className="px-6 py-4"><Skeleton className="h-4 w-24 bg-zinc-800" /></td>
                        <td className="px-6 py-4"><Skeleton className="h-4 w-20 bg-zinc-800" /></td>
                        <td className="px-6 py-4"><Skeleton className="h-4 w-24 bg-zinc-800" /></td>
                      ...
                      <tr key={job.id} className="hover:bg-zinc-800/20 transition-colors group">
                        ...
                        <td className="px-6 py-4">
                          <div className="flex gap-1">
                            {job.targets.map(t => (
                              <Badge key={t} variant="outline" className="text-[10px] py-0 border-zinc-700 bg-zinc-800/50 text-zinc-400">
                                {t}
                              </Badge>
                            ))}
                          </div>
