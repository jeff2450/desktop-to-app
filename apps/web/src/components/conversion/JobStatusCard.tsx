...
const STATUS_CONFIG: Record<ConversionStatus, { label: string; color: string; dot: string }> = {
  queued:       { label: "Queued",       color: "text-gray-400",  dot: "bg-gray-500" },
  detecting:    { label: "Detecting",    color: "text-blue-400",  dot: "bg-blue-500 animate-pulse" },
  planning:     { label: "Planning",     color: "text-blue-400",  dot: "bg-blue-500 animate-pulse" },
  transforming: { label: "Transforming", color: "text-yellow-400",dot: "bg-yellow-500 animate-pulse" },
  scaffolding:  { label: "Scaffolding",  color: "text-yellow-400",dot: "bg-yellow-500 animate-pulse" },
  installing:   { label: "Installing",   color: "text-orange-400",dot: "bg-orange-500 animate-pulse" },
  building:     { label: "Building",     color: "text-orange-400",dot: "bg-orange-500 animate-pulse" },
  packaging:    { label: "Packaging",    color: "text-purple-400",dot: "bg-purple-500 animate-pulse" },
...
const STAGE_PROGRESS: Record<ConversionStatus, number> = {
  queued: 0, detecting: 12, planning: 25, transforming: 38,
  scaffolding: 50, installing: 62, building: 75, packaging: 88,
  done: 100, failed: 0, cancelled: 0,
};
  ...
  if (compact) {
    return (
      <Link href={`/dashboard/conversions/${conversion.id}`}
        ...
        <span className="flex-1 text-sm text-white truncate">{conversion.name}</span>
        <span className={`text-xs ${cfg.color}`}>{cfg.label}</span>
        <span className="text-xs text-gray-600">{conversion.targets.join(", ")}</span>
      </Link>
    );
  }
  ...
  return (
    <Link href={`/dashboard/conversions/${conversion.id}`}
      ...
      <div className="flex items-start justify-between mb-3">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-0.5">
            {conversion.sourceUrl ?? conversion.sourceType} · {conversion.targets.join(", ")}
          </p>
        </div>
      ...
      {conversion.status === "failed" && conversion.errorMessage && (
        <p className="text-xs text-red-400 mt-2 truncate">{conversion.errorMessage}</p>
      )}
